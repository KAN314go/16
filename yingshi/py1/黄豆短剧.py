...
# 黄豆短剧 spider v2.0.1
# 变更：
#   1. 修复：服务端拒绝的集（金币/VIP/越界）不再回一个必然 403 的空地址导致黑屏，
#      改为直接返回明确原因。
#   2. 修复：不再把每集都假装成“免费/已购”——那只改本地字段、骗不过服务端，
#      还会让人看不出哪集要花钱；现在如实保留集类型。
#   3. 增强：集名标注【金币xx】/【VIP】/【积分】，列表与详情备注给出 免费x/金币x/VIPx 概览。
#   4. 增强：金币/VIP 集可播时自动走站点试看片段，并提示时长。
#   5. 优化：试看直链缓存延长；新增可选开关 {"verify":"1"} 播放前校验线路。
# 游客模式即可用，无需登录。
import gzip
import hashlib
import hmac
...
class Spider(BaseSpider):
    ...
    def init(self, extend=""):
        ...
        if str(cfg.get("verify") or "").strip() in ("1", "true", "on", "yes"):
            self.verify_playback = True
    ...
    def detailContent(self, ids):
        ...
        if eps:
            for i, ep in enumerate(eps, 1):
                ...
                play.append("%s%s$%s|%s" % (label, self._ep_tag(ep), vod_id, seq))
        else:
            play = ["第%s集$%s|%s" % (i, vod_id, i) for i in range(1, count + 1)]
        pay = self._pay_summary(eps)
        remark = data.get("update_label") or "全%s集" % count
        if pay:
            remark = "%s·%s" % (remark, pay)
        content = data.get("description") or data.get("summary") or name
        hint = self._pay_hint(eps)
        if hint:
            content = "%s\n\n【观看提示】%s" % (content, hint)
        vod = {"vod_id": vod_id, "vod_name": name, "vod_pic": self._pic(data), "type_name": data.get("category") or data.get("type") or "", "vod_year": "", "vod_area": "", "vod_remarks": remark, "vod_actor": "", "vod_director": "", "vod_content": content, "vod_play_from": self.name, "vod_play_url": "#".join(play)}
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # v2.0.1：服务端拒绝的集（金币/VIP/越界）不再拼一个必然 403 的自建地址，
        # 那种地址播放器只会黑屏、连原因都看不到。取不到线路就直接给明确提示。
        if not ordered:
        ...
        if isinstance(data, dict) and data.get("is_preview"):
            secs = self._int(data.get("preview_seconds"), 10)
            note = "试看片段 %s 秒 · 该集正片需在站点解锁" % secs
        return self._player(url, note)
    ...
    def _valid_media(self, url):
        ...
        headers["Content-Type"] = None
        try:
            with self._host_route(0):
                with self.session.get(url, headers=headers, timeout=(8, 20), stream=True) as response:
                    if response.status_code not in (200, 206):
                        self.last_hls_error = "media_http_%s" % response.status_code
                        return False
                    body = self._read_limited(response, 4096)
            if body.lstrip(b"\xef\xbb\xbf\x00\t\r\n ").startswith(b"#EXTM3U"):
                self.last_hls_error = ""
                return True
            if len(body) >= 12 and body[4:8] == b"ftyp":
                self.last_hls_error = ""
                return True
            self.last_hls_error = "invalid_media_body"
            return False
        except (requests.RequestException, _ApiError, ValueError, TypeError, OSError):
            self.last_hls_error = "media_request_error"
            return False
    ...
    def _url_expiry(self, url):
        ...
        try:
            ...
            pass
        if str(url).endswith(".mp4") or "/preview-" in str(url):
            return now + 6 * 3600
        return now + 120
    ...
    def _player(self, url, note=""):
        out = {"parse": 0, "playUrl": "", "url": url, "jx": 0, "header": self._media_headers()}
        if note:
            out["msg"] = note
        return out
    ...
    def _play_error(self, obj):
        if isinstance(obj, dict) and not self._api_ok(obj):
            code, detail = self._api_failure(obj)
            if code == "813005":
                return "本集需金币解锁（%s）" % (detail or "站点限制")
            if code == "813004":
                return "该剧为 VIP 专享（%s）" % (detail or "需开通会员")
            if code == "2002":
                return "该线路需登录后观看"
            return "播放接口拒绝请求（%s%s）" % (code, "：" + detail if detail else "")
        return "播放接口未返回可用播放线路"
    ...
    def _vod(self, item):
        ...
        remarks = item.get("update_label") or ("全%s集" % item.get("episode_count") if item.get("episode_count") else "")
        # 列表里就把付费标记摆出来，省得点进去才发现要金币/会员
        corner = str(item.get("corner") or "").strip()
        if corner and corner not in remarks:
            remarks = "[%s]%s" % (corner, remarks) if remarks else "[%s]" % corner
    ...
    def _annotate(self, d):
        # v2.0.1：以前这里是“把每集都改成免费/已购”，那只改本地字段、骗不过服务端，
        # 反而把真实付费状态抹掉，看的人不知道哪集要花钱。现在只做字段规范化，如实保留类型。
        eps = d.get("episodes")
        if isinstance(eps, list):
            for i, ep in enumerate(eps, 1):
                if isinstance(ep, dict) and not (ep.get("seq") or ep.get("episode") or ep.get("ep")):
                    ep["seq"] = i
        return d

    def _ep_kind(self, ep):
        return str((ep or {}).get("type") or "").strip().lower()

    def _ep_tag(self, ep):
        kind = self._ep_kind(ep)
        if kind == "vip":
            return "【VIP】"
        if kind == "coin":
            price = self._int((ep or {}).get("price"), 0)
            return "【金币%s】" % price if price else "【金币】"
        if kind == "points":
            return "【积分】"
        return ""

    def _pay_summary(self, eps):
        if not isinstance(eps, list) or not eps:
            return ""
        free = sum(1 for e in eps if self._ep_kind(e) in ("free", ""))
        coin = sum(1 for e in eps if self._ep_kind(e) == "coin")
        vip = sum(1 for e in eps if self._ep_kind(e) == "vip")
        parts = []
        if free:
            parts.append("免费%d" % free)
        if coin:
            parts.append("金币%d" % coin)
        if vip:
            parts.append("VIP%d" % vip)
        return "/".join(parts)

    def _pay_hint(self, eps):
        if not isinstance(eps, list) or not eps:
            return ""
        coin = sum(1 for e in eps if self._ep_kind(e) == "coin")
        vip = sum(1 for e in eps if self._ep_kind(e) == "vip")
        notes = []
        if coin:
            notes.append("标【金币】的集未解锁时只能看到 10 秒试看片段（站点服务端控制）")
        if vip:
            notes.append("标【VIP】的集需站点会员（未开通时部分集只能看到 10 秒试看）")
        if notes:
            notes.append("免费集可直接播放。")
        return "；".join(notes)
